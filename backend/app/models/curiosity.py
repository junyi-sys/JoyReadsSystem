...
class CuriosityEvent(Base, TimestampMixin):
    ...
    socratic_mode: Mapped[bool] = mapped_column(Boolean, default=False, comment="苏格拉底模式")
    follow_up_question: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="苏格拉底追问")
    child_response: Mapped[str | None] = mapped_column(Text, nullable=True, comment="孩子对追问的回答")
    theory_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("theory.id"), nullable=True, comment="关联的理论")

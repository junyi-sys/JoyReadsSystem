...
class PlanDay(Base, TimestampMixin):
    ...
    guide_text: Mapped[str | None] = mapped_column(Text, nullable=True, comment="导读语（纯文本，前端展示用）")
    seed_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("curiosity_seed.id"), nullable=True, comment="关联的好奇心种子")
    seed_question: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="种子问题文本")
    lesson_json: Mapped[str | None] = mapped_column(Text, nullable=True, comment="LLM 生成的完整教案 JSON (V2)")
